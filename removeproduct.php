...
<?php
// 1. Capture the ID from the form name="product_id"
...
// 3. Run the Delete query using the Primary Key
// We don't use quotes around $pid because it is a Primary Key (Integer)
$sql = "DELETE FROM addproduct WHERE product_id = $pid";

if (mysqli_query($conn, $sql)) {
    // 4. Check if any row was actually deleted
    if (mysqli_affected_rows($conn) > 0) {
        echo "<h2 class='success-text'>Product Removed!</h2>";
        echo "<p>Product with ID <b>$pid</b> was successfully deleted.</p>";
    } else {
        echo "<h2 class='error-text'>Not Found</h2>";
        echo "<p>No product exists with ID <b>$pid</b>. Please check your database.</p>";
    }
} else {
